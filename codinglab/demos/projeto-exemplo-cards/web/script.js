...
window.onload = function(){
    ...
    videos.map(video => {
      ...
      cardClone.querySelector(".card__title").innerHTML = video.title;
      cardClone.querySelector(".card__info > p.text--medium").innerHTML = video.duration;
    ...
    cards.forEach(card => {
      card.addEventListener("click", () => {
        modal.querySelector("iframe").src = `https://www.youtube.com/embed/${card.getAttribute("id")}`
    ...
    menuToggle.addEventListener("click", () => {
      ...
      document.querySelector("body").style.overflow = toggle ? "hidden" : "initial";
      
      menuSection.classList.toggle("on", toggle);
      toggle = !toggle;
    })
}
